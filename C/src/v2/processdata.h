...
Index Bias(Index t_r) {
	...
	Scale(0.5, Inc(1, Scale(-1.0, Cos(Scale(2.0*M_PI/((float) COLS), Iota(t_r), t_r), t_r), t_r), t_r), t_r);
...
Index WaveCoefficients(float freq) {
	...
	// ROW 16 is used to store the scalar coefficients
	// ROW 18 is a temp row used by multiple calculations
	Coord m0 = POS(0, 16);
	Index m0_row = ROW(18);
	...
	Coord wave_energy_ratio = POS(9, 16);
	DivCell(te, tp, wave_energy_ratio);

	Coord Tz = POS(10, 16);
	SqrtCell(DivCell(m0, m2, Tz), Tz);

	Coord PeakPSD = POS(11, 16);
	MaxCell(a0, PeakPSD);


	// doesn't match python
	Coord Dp_true = POS(12, 16);
